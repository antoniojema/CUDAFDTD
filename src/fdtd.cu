...
HostField::HostField(const Field _ft, const Orientation _or, const Box<ssize_t,3> _sim_bounds) :
    eh(_ft), xyz(_or)
{
    if (_ft == Field::None || _or == Orientation::None) {
        //TODO: ERROR
    }
    ...
    switch (_or) {
    case (Orientation::X):
        add[0] = (_ft == Field::E) ? 0 : 1;
        add[1] = (_ft == Field::E) ? 1 : 0;
        add[2] = (_ft == Field::E) ? 1 : 0;
        break;
    case (Orientation::Y):
        add[0] = (_ft == Field::E) ? 1 : 0;
        add[1] = (_ft == Field::E) ? 0 : 1;
        add[2] = (_ft == Field::E) ? 1 : 0;
        break;
    case (Orientation::Z):
        add[0] = (_ft == Field::E) ? 1 : 0;
        add[1] = (_ft == Field::E) ? 1 : 0;
        add[2] = (_ft == Field::E) ? 0 : 1;
        break;
    }
    ...
    this->f = new real_t[this->getAllocSize()];
    for (size_t i = 0; i < this->getAllocSize(); i++) {
        this->f[i] = 1.;
    }
}

real_t& HostField::getElem(const ssize_t _i, const ssize_t _j, const ssize_t _k) {
    ...
    return this->f[i * sy*sz + j * sz + k];
}

const real_t& HostField::getElem(const ssize_t _i, const ssize_t _j, const ssize_t _k) const {
    size_t i = _i - this->alloc[0][0];
    size_t j = _j - this->alloc[0][1];
    size_t k = _k - this->alloc[0][2];
    size_t sz = this->getAllocSizeInDim(Orientation::Z);
...
size_t HostField::getAllocSize() const {
    size_t total_size = 1;
    for (size_t i = 0; i < 3; i++) {
        total_size *= (size_t)(this->alloc[1][i] - this->alloc[0][i]);
    }
    return total_size;
}

size_t HostField::getAllocSizeInDim(const Orientation _or) const {
...
DeviceField::DeviceField(HostField& _hf) :
